...
export const PlayersHeader: FC<PlayersHeaderProps> = ({ players, currentPlayerId, leadingPlayerId, isExpanded, onToggleExpand }) => {
  const shouldTruncate = players.length >= 5;
  const { calculateTotal } = useScoreCalculations();
  const totalScore = (player: Player) => calculateTotal(player);
  ...
  return (
    <div className="z-40 grid gap-x-2" style={{ gridTemplateColumns: '44px minmax(0, 1fr)' }}>
      <button
        ...
        <div className="-m-1">{isExpanded ? <ChevronLeft className="w-6 h-6 stroke-[1.5]" /> : <ChevronRight className="w-6 h-6 stroke-[1.5]" />}</div>
      </button>
          ...
          return (
            <div key={player.id} className="relative min-w-0">
              ...
              `}>
                ...
                <div className={cn('font-medium mt-0.5 w-full text-center px-1', 'text-xs', isCurrentPlayer ? 'text-purple-800' : 'text-purple-600')}>
